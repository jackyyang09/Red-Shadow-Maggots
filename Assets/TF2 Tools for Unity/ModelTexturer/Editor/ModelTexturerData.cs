...
namespace TFTools
{
    public class ModelTexturerData : ScriptableObject
    {
        ...
        public MatNameMode matNameMode = MatNameMode.UseMaterialName;
    ...
    public class ModelTexturerWindow : TFToolsSerializedEditorWindow<ModelTexturerData, ModelTexturerWindow>
    {
        ...
        SerializedProperty matNameMode;
        ...
        protected override void DesignateSerializedProperties()
        {
            ...
            matNameMode = SerializedObject.FindProperty(nameof(matNameMode));
        ...
        void RenderModelTexturerTool()
        {
            ...
            EditorGUILayout.PropertyField(matNameMode, new GUIContent("Name Detection Mode", 
                "Use Material Name - Will attempt to match base textures to the generated material based " +
                "on the name of the original material in this mesh. \n\n" +
                "Use GameObject Name - Will attempt to match base textures to the generated material based " +
                "on the name of the GameObject this MeshRenderer is attached to. Don't use this if this " +
                "renderer contains multiple materials."
                ));
            ...
            using (new EditorGUI.DisabledScope(localObject == null && localRenderer == null))
            {
                if (GUILayout.Button("Apply Textures to Model(s)"))
                {
                    ...
                    for (int i = 0; i < allRenderers.Count; i++)
                    {
                        ...
                        {
                            var name = allRenderers[i].sharedMaterials[j].name;
                            if (!materialsToGen.Contains(name)) materialsToGen.Add(name);
                        }
                    }
                    ...
                    if (vtfExists)
                    {
                        OnShellProcessComplete += () =>
                        {
                            ...
                            TFToolsAssPP.pathOfInterest = textureOutputFolderPath.stringValue;
        ...
        void ApplyTexturesToModel(List<Texture> tex)
        {
            ...
            for (int x = 0; x < allRenderers.Count; x++)
            {
                ...
                for (int i = 0; i < sharedMats.arraySize; i++)
                {
                    ...
                    string matName = "";
                    switch (matNameMode.enumValueIndex)
                    {
                        case (int)ModelTexturerData.MatNameMode.UseMaterialName:
                            matName = mat.name;
                            break;
                        case (int)ModelTexturerData.MatNameMode.UseGameObjectName:
                            matName = allRenderers[i].name;
                            break;
                    }

                    EditorUtility.DisplayProgressBar(
                        "Applying textures to " + allRenderers[x].name,
                        matName, (float)i / (float)sharedMats.arraySize);

                    string baseTextureName = string.Empty, normalName = string.Empty, specularName = string.Empty;
                    string vmtPath = Path.Combine(textureInputFolderPath.stringValue, matName + ".vmt");
                    bool hasVMT = false;
                    bool painted = false;
                    float btcob = -1;
    ...
    class TFToolsAssPP : AssetPostprocessor
    {
        ...
        public static string pathOfInterest;

        void OnPreprocessTexture()
        {
            if (pathOfInterest == null) return;
            if (assetPath.Contains(pathOfInterest))
            {
